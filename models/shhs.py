    ...
    for i in tqdm(range(nb_patients)):
        ...
        if not os.path.exists(save_file):
            try:
                ...
                data,label_new = np.expand_dims(examples_patient_i, axis=2), stages[3:]

                if extend:
                    # create each data as + previous 3 segments
                    data_new = None
                    for pat_eeg_idx in range(data.shape[0]):
                        if pat_eeg_idx > 2:
                            data_temp = np.reshape(data[pat_eeg_idx - 3:pat_eeg_idx + 1, :, :], (1, 15000, 1))
                            if data_new is None:
                                data_new = data_temp
                            else:
                                data_new = np.concatenate((data_new, data_temp), axis=0)

                    data = data_new,label_new
