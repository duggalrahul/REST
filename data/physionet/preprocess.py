...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./output_fz/",
                        help="Directory where to save outputs.")
    parser.add_argument("--extend", type=bool, default=False,
                        help="Whether to concatenate 4 time stamps.")
    ...
    for i in range(len(psg_fnames)):
        ...
        if args.extend:
            x_extended = None
            for idx,eeg in enumerate(range(x.shape[0])):
                if idx>2:
                    temp = np.array([x[idx-3,:,:],x[idx-2,:,:],x[idx-1,:,:],x[idx,:,:]]).reshape((1,12000,1))
                    x_extended = np.concatenate([x_extended,temp],axis=0) if x_extended is not None else temp

            x,y = x_extended,y[3:]
            print("Data after extension: {}, {}".format(x.shape, y.shape))
