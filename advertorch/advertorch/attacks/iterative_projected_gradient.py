...
def perturb_iterative(xvar, yvar, predict, nb_iter, eps, eps_iter, loss_fn,
                      ...
                      clip_min=0.0, clip_max=1.0, uniform_sample=False):
    """
    ...
    :param uniform_sample: (optional float) flag to enable sampling of epsilon.
...
class PGDAttack(Attack, LabelMixin):
    ...
    def __init__(
            ...
            ord=np.inf, targeted=False, uniform_sample=False):
        """
        ...
        :param uniform_sample: if epsilon needs to be sampled from U(-eps,+eps)
        """
    ...
    def perturb(self, x, y=None):
        ...
        rval = perturb_iterative(
            ...
            clip_max=self.clip_max, delta_init=delta, uniform_sample=self.uniform_sample)
...
class LinfPGDAttack(PGDAttack):
    def __init__(
            self, predict, loss_fn=None, eps=0.3, nb_iter=40,
            eps_iter=0.01, rand_init=True, clip_min=0., clip_max=1.,
            targeted=False, uniform_sample=False):
        ord = np.inf
        super(LinfPGDAttack, self).__init__(
            predict, loss_fn, eps, nb_iter, eps_iter, rand_init,
            clip_min, clip_max, ord, targeted, uniform_sample)
