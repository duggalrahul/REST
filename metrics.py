# code from https://raw.githubusercontent.com/simochen/model-tools/master/pytorch_tools.py
import torch
import torchvision
...
def print_model_param_flops(model=None, input_res=[224, 224, 3], multiply_adds=True):
    ...
    def conv_hook(self, input, output):
        if isinstance(self,nn.Conv2d):
            ...
            output_height,output_width = 1,output_length
    ...
    def pooling_hook(self, input, output):
        if isinstance(self,torch.nn.MaxPool2d) or isinstance(self, torch.nn.AvgPool2d):
            batch_size, input_channels, input_height, input_width = input[0].size()
            output_channels, output_height, output_width = output[0].size()
            kernel_ops = self.kernel_size * self.kernel_size
        elif isinstance(self,torch.nn.MaxPool1d) or isinstance(self, torch.nn.AvgPool1d):
            batch_size,input_channels,input_length = input[0].size()
            output_channels,output_length = output[0].size()
            output_height,output_width = 1,output_length
            kernel_ops = self.kernel_size[0] if isinstance(self.kernel_size,tuple) else self.kernel_size
        bias_ops = 0
        params = 0
    ...
    def foo(net):
        ...
        if not childrens:
            if isinstance(net, torch.nn.Conv2d) or isinstance(net, torch.nn.ConvTranspose2d) or isinstance(net,torch.nn.Conv1d):
                net.register_forward_hook(conv_hook)
            if isinstance(net, torch.nn.Linear):
                net.register_forward_hook(linear_hook)
            if isinstance(net, torch.nn.BatchNorm2d) or isinstance(net, torch.nn.BatchNorm1d):
                net.register_forward_hook(bn_hook)
            if isinstance(net, torch.nn.ReLU):
                net.register_forward_hook(relu_hook)
            if isinstance(net, torch.nn.MaxPool2d) or isinstance(net, torch.nn.AvgPool2d) or \
                    isinstance(net, torch.nn.MaxPool1d) or isinstance(net, torch.nn.AvgPool1d):
    ...
    foo(model)
    if len(input_res) == 3:
        input = Variable(torch.rand(input_res[2],input_res[1],input_res[0]).unsqueeze(0), requires_grad = True)
    elif len(input_res) == 2:
        input = Variable(torch.rand(input_res[1], input_res[0]).unsqueeze(0), requires_grad=True)
    else:
